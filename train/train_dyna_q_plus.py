...
from agents.dyna_q_plus import dyna_q_plus
...
def save_plot(rewards, label, output_dir):
    ...
    filename = f"dyna_q_plus_{label.replace(' ', '_').replace('=', '').replace(',', '')}.png"
...
def main(args):
    ...
    for cfg in configs:
        ...
        kappa = cfg["kappa"]
        label = f"α={alpha}, γ={gamma}, ε-decay={epsilon_decay}, plan={planning_steps}, κ={kappa}"

        print(f"Training Dyna-Q+ with: {label}")
        q_table, rewards = dyna_q_plus(
            env=env,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="configs/dyna_q_plus.json")
    parser.add_argument("--episodes", type=int, default=5000)
    parser.add_argument("--output", type=str, default="results/dyna_q_plus")
