...
def evaluate_all(results):
    ...
    print("\n=== n-step SARSA Config Performance Ranking ===")
    for i, (label, avg) in enumerate(summary, 1):
        print(f"{i}. {label}: Avg Final Reward = {avg:.2f}")
    return summary

def plot_all_curves(results, output_path="results/n_step_sarsa/all_configs_comparison.png"):
    plt.figure(figsize=(12, 6))
    for label, rewards in results.items():
        smoothed = moving_average(rewards)
        plt.plot(smoothed, label=label)
    plt.title("n-step SARSA: All Configs Comparison")
    plt.xlabel("Episode")
...
def main():
    results_dir = "results/n_step_sarsa"
    rewards = {}
