...
def evaluate_all(results):
    ...
    print("\n=== Monte Carlo Control Config Performance Ranking ===")
    for rank, (label, avg_reward) in enumerate(summary, 1):
        print(f"{rank}. {label}: Final Avg Reward = {avg_reward:.2f}")

    return summary

def plot_all_curves(results, output_path="results/monte_carlo/all_configs_comparison.png"):
    ...
    plt.title("Monte Carlo Control: All Configs Comparison")
    plt.legend()
    plt.grid()
    plt.tight_layout()
